...
GATEWAY_CONFIG="${GATEWAY_CONFIG:-tooling/config/gateway.yaml}"
...
(
  cd services/recognition-api
  uv run uvicorn main:app --reload --host "$APP_HOST" --port "$APP_PORT"
) &
APP_PID="$!"
go run ./services/api-gateway/cmd/gateway --config "$GATEWAY_CONFIG" &
GW_PID="$!"
(cd clients/operator-console && bun run dev -- --host "$FRONTEND_HOST" --port "$FRONTEND_PORT") &
FE_PID="$!"
