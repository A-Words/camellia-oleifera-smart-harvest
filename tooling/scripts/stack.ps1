...
param(
    ...
    [int]$AppPort = 8000,
    [string]$GatewayConfig = "tooling/config/gateway.yaml",
...
try {
    ...
    $appProc = Start-Process -FilePath "uv" -ArgumentList $appArgs -WorkingDirectory "services/recognition-api" -PassThru

    $gatewayArgs = @("run", "./services/api-gateway/cmd/gateway", "--config", $GatewayConfig)
    ...
    $frontendProc = Start-Process -FilePath "bun" -ArgumentList $frontendArgs -WorkingDirectory "clients/operator-console" -PassThru

    Write-Host "[stack] app pid=$($appProc.Id), gateway pid=$($gatewayProc.Id), frontend pid=$($frontendProc.Id)"
    ...
    Write-Host "[stack] frontend exited with code $($frontendProc.ExitCode)"
    if (-not $appProc.HasExited) {
        Stop-Process -Id $appProc.Id -Force
    }
    if (-not $gatewayProc.HasExited) {
