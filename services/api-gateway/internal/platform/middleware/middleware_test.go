...
func TestAuthAllowsPublicHealthPathWithoutKey(t *testing.T) {
	cfg := config.AuthConfig{Enabled: true, APIKeys: []string{"secret-key"}}
	mw := Auth(cfg, slog.Default())
	handler := mw(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	...
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Errorf("expected 200 for public health path, got %d", rec.Code)
	}
}
...
func TestAuthStillRequiresKeyForProtectedPath(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/recognition/image", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusUnauthorized {
...
func TestRateLimitExcludesHealthz(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/recognition/image", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("first /v1/recognition/image: expected 200, got %d", rec.Code)
	}
	...
	handler.ServeHTTP(rec2, req)
	if rec2.Code != http.StatusTooManyRequests {
		t.Errorf("/v1/recognition/image after exhaust: expected 429, got %d", rec2.Code)
	}
...
func TestLoggingPreservesHijacker(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/recognition/stream", nil)
	handler.ServeHTTP(rec, req)
