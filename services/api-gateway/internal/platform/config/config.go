...
func Defaults() Config {
	return Config{
		...
		DB: DBConfig{
			Driver:           "sqlite",
			DSN:              filepath.Join("mlops", "artifacts", "data", "gateway.db"),
			MaxOpenConns:     10,
			MaxIdleConns:     5,
...
// If the path is empty, it tries the CAMELLIA_GATEWAY_CONFIG env var,
// then falls back to tooling/config/gateway.yaml relative to the working directory.
func Load(path string) (Config, error) {
	if path == "" {
		path = os.Getenv("CAMELLIA_GATEWAY_CONFIG")
	}
	if path == "" {
		path = filepath.Join("tooling", "config", "gateway.yaml")
	}
